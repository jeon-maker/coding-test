...
def where_to_go( y, x , direction):
    ...
    turn_count +=1
    if direct == north:
        if boolean(array2[y-1][x] == 0) & boolean((b-1,a) not in array3): #육지이고 안가본 곳이라면
            y = y-1
            x = x
            count_ +=1
            array3.append((y,x))
            ...
            pass
    if direct == south:
        if boolean(array2[y+1][x] == 0) & boolean((b+1,a) not in array3):
            y = y+1
            x = x
            count_ +=1
            array3.append((y,x))
            ...
            pass
    if direct == east:
        if boolean(array2[y][x+1] == 0) & boolean((b,a+1) not in array3):
            y = y
            x = x+1
            count_ += 1
            array3.append((y,x))
